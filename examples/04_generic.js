/* This example shows how to draw generic text metadata, which will just be
shown stringified below the timeline. */

import React from 'react';
import ReactDOM from 'react-dom';
import {VGrid, Table, Database, IntervalSet, Interval, Bounds, BoundingBox,
  SpatialType_Temporal, Metadata_Generic} from '@wcrichto/vgrid';
  ...
  interval_sets: [{
    ...
    interval_set: new IntervalSet([new Interval(
      new Bounds(0, 10),
      {
        ...
        metadata: {
          'generic_metadata': new Metadata_Generic(
            'this will be drawn below the timeline when expanded'
          ),
          'other_generic_metadata': new Metadata_Generic({
